...
| Skill | Module | Trigger | Purpose |
|---|---|---|---|
| [marsad-seed](marsad-seed.md) | MARSAD | `/marsad-seed` | Stage 0: import historical price CSV/JSON to pre-warm forecasting model (run before DISCOVER) |
...
MARSAD is an additive module — does not modify any Phase 1/2/3 stable skill or file. Data is strict_local (gitignored). Entry point: `MARSAD__flight_radar/radar/main.py`. Scheduled: 06:00 UTC daily (MONITOR → ALERT → FORECAST). Run SEED → DISCOVER before first monitoring cycle.
